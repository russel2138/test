...
select_runtime() {
  ...
  if [[ "$EMU_NAME" == "stock" && "$GAME" == "$ROOT/game.jar" ]]; then
    # Preserve the existing Raven login/RMS path exactly.
...
choose_runtime() {
  ...
  # Existing Raven installs keep working without migration.
...
case "${1:-}" in
  ...
  _game_loop)
    ...
    ;;
  *)
    echo "Usage: $0 {list|start [emulator game]|stop|restart [emulator game]|status|log|log-follow|game-restart|loop-check}"
    exit 1
    ;;
esac
